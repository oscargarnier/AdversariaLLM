...
def test_crescendo_attack():
    ...
    # to use gpt
    # cfg.crescendo.attack_model.use_api = True
    # cfg.crescendo.attack_model["api_model_name"] = "gpt-4o"

    # Manually set the seed to fix interpolation issue
    cfg.crescendo.seed = 0
    ...
    cfg.crescendo.generation_config = generation_config
    cfg.crescendo.max_new_tokens = 256

    # Override max_rounds to 2 for fast testing
    cfg.crescendo.max_rounds = 2

    try:
        attack = CrescendoAttack(cfg.crescendo)
        ...
        assert hasattr(result.runs[0], 'criteria')
        assert hasattr(result.runs[0], 'attacker_conv')
